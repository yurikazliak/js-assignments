...
function unbracketTag(str) {
    return str.slice(0, str.indexOf('<')) + str.slice(str.indexOf('<') + 1, str.indexOf('>'));
}
...
function getRectangleString(width, height) {
    ...
    let centerRow = '│' + rowEmpty + '│\n';
    for (let i = 1; i < height - 1; i++) {
        col += centerRow;
    }
    return topRow + col + bottomRow;
}
...
function encodeToRot13(str) {
    ...
    let codeString = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm !?,.';
    let recodedStr = '';
    for (let i = 0; i < str.length; i++) {
        recodedStr += codeString.charAt(alphabet.indexOf(str.charAt(i)));
    }
    return recodedStr;
}
